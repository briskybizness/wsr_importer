...
def parse_manual_column_input(text: str) -> List[str]:
    if not text:
        return []
    parts = re.split(r"[\n,]", text)
                ...
                rows.append(
                    {
                        ...
                        "Description": "\n".join([line for line in [
                            ...
                            f"WSR Sheet: {sheet_name}",
                            f"WSR Row: {row_index + 2}",
...
with st.sidebar:
    ...
    include_blank_dates = st.checkbox("Include tasks even when the pipeline date is blank", value=False)

    st.header("Sheets")
    included_sheets = st.multiselect(
        "Choose sheets to convert",
        options=usable_sheet_names,
        default=[sheet for sheet in usable_sheet_names if normalize_sheet_name(sheet) in {"shots", "assets"}],
    )
...
selected_task_columns_by_sheet: Dict[str, List[str]] = {}
manual_task_columns_by_sheet: Dict[str, List[str]] = {}
pipeline_step_by_sheet: Dict[str, str] = {}
...
for sheet_name in included_sheets:
    ...
    with st.expander(f"{sheet_name} ({entity_type})", expanded=True):
        ...
        pipeline_step_value = st.text_input(
            ...
            value=default_pipeline_step,
            key=f"pipeline_step_{sheet_name}",
        ...
        manual_column_text = st.text_area(
            f"Manual pipeline columns for {sheet_name}",
            value="\n".join(default_pipeline_columns),
            key=f"manual_task_columns_{sheet_name}",
            help="Enter the exact pipeline date column names separated by commas or line breaks. Only columns you enter here will be available. Excluded columns like Next Submission Date will still be ignored.",
        )
        manual_columns_requested = parse_manual_column_input(manual_column_text)
        manual_columns_found = [col for col in df.columns if str(col).strip() in manual_columns_requested and str(col).strip() not in EXCLUDED_TASK_COLUMNS]
        manual_columns_missing = [col for col in manual_columns_requested if col not in [str(df_col).strip() for df_col in df.columns] or col in EXCLUDED_TASK_COLUMNS]
        manual_task_columns_by_sheet[sheet_name] = manual_columns_found

        available_task_columns = []
        for col in manual_columns_found:
            if col not in available_task_columns:
                available_task_columns.append(col)

        if available_task_columns:
            default_columns = available_task_columns
            selected_columns = st.multiselect(
                f"Task columns for {sheet_name}",
                options=available_task_columns,
                default=default_columns,
                key=f"task_columns_{sheet_name}",
            )
            selected_task_columns_by_sheet[sheet_name] = selected_columns
        else:
            st.warning("No pipeline date columns have been entered for this sheet.")
            selected_task_columns_by_sheet[sheet_name] = []

        if manual_columns_missing:
            st.warning("These manual columns were not found exactly as typed: " + ", ".join(manual_columns_missing))

        preview_task_columns = []
        for col in manual_task_columns_by_sheet[sheet_name]:
            if col in df.columns and col not in preview_task_columns:
                preview_task_columns.append(col)
        preview_columns = [col for col in [entity_column, *preview_task_columns, "Vendor Status", "Vendor Note"] if col in df.columns]

        if preview_columns:
            st.dataframe(dataframe_for_display(df[preview_columns]), width="stretch")
...
st.dataframe(dataframe_for_display(tasks_df), width="stretch", height=520)

csv_bytes = tasks_df.to_csv(index=False).encode("utf-8")
st.download_button(
...
with st.expander("Column notes", expanded=False):
    st.write(
        "This CSV is intentionally generic. You can map `Task Name`, `Pipeline Step`, `Milestone`, `Entity Type`, `Link`, `Delivery Date`, `Description`, and the WSR tracking columns during your ShotGrid import test."
    )
